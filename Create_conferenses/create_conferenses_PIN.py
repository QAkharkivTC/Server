...
def creat_payload (id_conf, y, k, max_participants, max_podiums):
    ...
    payload = {
        ...
        "type": str(y),
        "topic": "Conf with PIN 321"+str(id_conf)+"",
                ...
                if ty2_conf%2 == 0:                                 #публичная
                    ...
                    for t in range(3):                              #выбираем тип расписания для типа конференции
                        if t == 0:                                  #-1 - without schedule
                            ...
                            payload.update({"pin": "000"+str(id_conf)+""})
                            payload.update({"topic": "Conf with PIN "+str(id_conf)+" " +mode_conf + shed + type_conf_name+" "+str(max_podiums)+"*"+str(max_participants)+""})
                        elif t == 1:                                #0 - weekly
                            shed = "/с расписанием/"
                            type_s = str(t)
                            sch1={}
