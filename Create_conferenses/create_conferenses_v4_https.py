...
def creat_payload (id_conf, y, k, max_participants, max_podiums):
    ...
    payload = {
        ...
        "owner": "user11@aa11h.trueconf.name",                      #указать домен сервера (при использовании с другими серверами)
...
def send_response (payload, id_conf, localhost, token):
    '''
    ...
    url = 'https://'+localhost+'/api/v3.1/conferences?access_token='+token+''
            ...
            for ty2_conf in range (2):                              #выбираем вид конференции (внутренняя, публичная)
                if ty2_conf%2 == 0:                                 #публичная
                    ...
                    for t in range(3):                              #выбираем тип расписания для типа конференции
                        ...
                        send_response (payload, id_conf, localhost, token)
