...
def create_users(N, id_u, email_u, localhost, token):
    ...
    for k in range(N):
        ...
        r = requests.post('https://'+localhost+'/api/v3.1/users?access_token='+token+'', data= {
             "id": ""+id_u+str(k+1)+"",
...
def create_groups(G, localhost, token):
    ssl._create_default_https_context = ssl._create_unverified_context
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    ...
    for k in range(G):
        start = time.monotonic()
        r = requests.post('https://'+localhost+'/api/v3.1/groups?access_token='+token+'', data=  {"display_name": "Group"+str(k+1)+""}, verify=False)
        r.status_code
...
def add_cotact_to_users(N, id_u, localhost, token):
    ssl._create_default_https_context = ssl._create_unverified_context
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    ...
    for i in range(len(contact_list)):
        for user in range(N):
            start = time.monotonic()
            r = requests.post('https://'+localhost+'/api/v3.1/users/'+id_u+str(user+1)+'/addressbook?access_token='+token+'', data= contact_list[i], verify=False)
            r.status_code
...
def add_sub_contacts(N, id_u, K, localhost, token):
    ssl._create_default_https_context = ssl._create_unverified_context
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    ...
    for i in range(K):
        ...
        for user in range(N):
            start = time.monotonic()
            r = requests.post('https://'+localhost+'/api/v3.1/users/'+id_u+str(user+1)+'/addressbook?access_token='+token+'', data= contact_list[0], verify=False)
            r.status_code
...
#add_cotact_to_users(N, id_u, token)

def add_user_to_group(N, G, id_u, localhost, token):
    ssl._create_default_https_context = ssl._create_unverified_context
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    
                                                #N - users G - groups
    uig = N//G                                  #количество пользователей в группах
    ...
    for f in range(G):
            for y in range(uig):
                ...
                start = time.monotonic()
                if f < 9:
                    r = requests.post('https://'+localhost+'/api/v3.1/groups/000'+str(f+1)+'/users?access_token='+token+'', data= {"user_id":""+id_u+str(u)+""}, verify=False)
                elif (f>=9 and f<99):
                    r = requests.post('https://'+localhost+'/api/v3.1/groups/00'+str(f+1)+'/users?access_token='+token+'', data= {"user_id":""+id_u+str(u)+""}, verify=False)
                elif (f>=99 and f<999):
                    r = requests.post('https://'+localhost+'/api/v3.1/groups/0'+str(f+1)+'/users?access_token='+token+'', data= {"user_id":""+id_u+str(u)+""}, verify=False)
                else:
                    r = requests.post('https://'+localhost+'/api/v3.1/groups/'+str(f+1)+'/users?access_token='+token+'', data= {"user_id":""+id_u+str(u)+""}, verify=False)
                r.status_code
...
#create_groups(G, localhost, token)                    # создает N групп
create_users(N, id_u, email_u, localhost, token)      # создает N пользователей
#add_user_to_group(N, G, id_u, localhost, token)       # распределяет N пользователей по N групп 
#add_cotact_to_users(N, id_u, localhost, token)        # добавляет контакты в АК пользователей (39 шт sip/h323/rtsp)
#add_sub_contacts(N, id_u, K, localhost, token)        # добавляет N псевдоконтактов в АК пользователя
