...
def create_users(N, id_u, email_u):
    ...
    for k in range(N):
        ...
        r = requests.post('http://localhost/api/v3.1/users', data= {
             "id": ""+id_u+str(k+1)+"",
...
def create_groups(G):
    ...
    for k in range(G):
        start = time.monotonic()
        r = requests.post('http://localhost/api/v3.1/groups', data=  {"display_name": "Group"+str(k+1)+""})
        r.status_code
...
def add_cotact_to_users(N, id_u):
    ...
    for i in range(len(contact_list)):
        for user in range(N):
            start = time.monotonic()
            r = requests.post('http://localhost/api/v3.1/users/'+id_u+str(user+1)+'/addressbook', data= contact_list[i])
            r.status_code
...
def add_sub_contacts(N, id_u, K):
    for i in range(K):
        ...
        for user in range(N):
            start = time.monotonic()
            r = requests.post('http://localhost/api/v3.1/users/'+id_u+str(user+1)+'/addressbook', data= contact_list[0])
            r.status_code
...
def add_user_to_group(N, G, id_u):
    ...
    for f in range(G):
            for y in range(uig):
                ...
                start = time.monotonic()
                if f < 9:
                    r = requests.post('http://localhost/api/v3.1/groups/000'+str(f+1)+'/users', data= {"user_id":""+id_u+str(u)+""})
                elif (f>=9 and f<99):
                    r = requests.post('http://localhost/api/v3.1/groups/00'+str(f+1)+'/users', data= {"user_id":""+id_u+str(u)+""})
                elif (f>=99 and f<999):
                    r = requests.post('http://localhost/api/v3.1/groups/0'+str(f+1)+'/users', data= {"user_id":""+id_u+str(u)+""})
                else:
                    r = requests.post('http://localhost/api/v3.1/groups/'+str(f+1)+'/users', data= {"user_id":""+id_u+str(u)+""})
                r.status_code
