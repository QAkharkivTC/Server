...
cookies_value = 'ea573dbe280b4235ac1f3c8fb16bb36d'      #cookies со страницы администратора сервера
...
def get_stream_id(ip, cid, token):
    #получаем stream_id конференции в которую будем приглашать и удалять пользователя
...
def get_instans_partisipant(user, cid, ip, token, login_admim, password_admin, cookies_value):
    #получаем инстанс пользователя которого будем приглашать и удалять
...
def remove_partisipant(ip, cid, user, server_name, token, login_admim, password_admin, cookies_value):
    ssl._create_default_https_context = ssl._create_unverified_context
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    
    stream_id = get_stream_id(ip, cid, token)
    remove_user = user+'@'+server_name
    instans = get_instans_partisipant(user, cid, ip, token, login_admim, password_admin, cookies_value)
...
def repeat_invite(N):
    ...
    for i in range(N):
        ...
        time.sleep(1)
        remove_partisipant(ip, cid, user, server_name, token, login_admim, password_admin, cookies_value)
        time.sleep(1)
